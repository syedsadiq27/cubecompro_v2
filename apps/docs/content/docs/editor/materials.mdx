...
`SET_MATERIAL` binds a choice value and model target to a project `MaterialAsset`. The current Editor mappings panel loads available project materials and can change the material asset on a hydrated material binding.
...
3. Select a project material asset.
...
4. Save, reload, and confirm the same material asset remains selected.
5. Resolve the draft through the API and confirm the effect returns the expected `materialAssetId`.
