...
For a valid selection, the visual response includes a `SET_MATERIAL` action with `targetKey`, `materialAssetId`, `documentUrl`, and JSON value data. The renderer loads that immutable material and applies it to the target slot.
