...
import { LiveCommerceProof } from '@/components/products/workspace/live-commerce-proof';
...
export function CommerceTab({
  ...
  return (
    <div className="space-y-4">
      {detail ? (
        <LiveCommerceProof projectId={projectId} detail={detail} />
      ) : null}
      ...
      {!view ? (
        <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-pure)] p-6 shadow-xs space-y-3">
          ...
          <p className="text-[13px] text-[var(--text-secondary)] max-w-xl">
            This product is not linked to a Shopify import yet. Connect a shop
            and import a catalog product to create identity dimensions and
            mappings. Hosted CubeCom commerce uses the live preview above.
          </p>
          <Button
            as={Link}
