...
export function materialAssetRevisionDocumentUrl(
  apiUrl: string,
  materialAssetRevisionId: string
): string {
  return `${apiUrl.replace(/\/$/, '')}/documents/material-revisions/${materialAssetRevisionId}`;
}

export function textureAssetRevisionDocumentUrl(
  apiUrl: string,
  textureAssetRevisionId: string
): string {
  return `${apiUrl.replace(/\/$/, '')}/documents/texture-revisions/${textureAssetRevisionId}`;
}
