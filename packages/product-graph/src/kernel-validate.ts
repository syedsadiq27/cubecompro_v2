import type { Selection } from './kernel.js';
import { isSelectionComplete } from './kernel.js';
...
/**
 * Availability with replacement semantics: candidateSelection replaces the
 * choice under test, then asks whether any valid complete configuration extends it.
 */
export function deriveAvailability(
  ...
  for (const choice of choices) {
    ...
    for (const value of choice.values) {
      ...
      byValue[value.key] = canExtendToValidComplete(
        candidate,
        choices,
        constraints
      );
    }
    availability[choice.key] = byValue;
  }
